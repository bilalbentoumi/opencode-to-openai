...
import type { AssistantMessage, Part, SessionPromptData } from '@opencode-ai/sdk';
import type { Backend } from './backend.js';
import type { Config } from './config.js';
import { runWithForwardedAuth } from './authContext.js';
import {
  buildChunk,
  buildCompletion,
  extractParts,
...
const writeSse = (res: Response, payload: unknown) => res.write(`data: ${JSON.stringify(payload)}\n\n`);
...
export function createApp(config: Config, backend: Backend): Express {
  ...
  app.use((req: Request, _res: Response, next: NextFunction) => {
    runWithForwardedAuth(config.forwardAuth ? req.headers.authorization : undefined, next);
  });

  // Bearer-token auth (disabled when no apiKey is configured).
  ...
  let modelCache: { models: Array<{ id: string; owned_by: string; name: string }>; at: number } | null = null;
  ...
  async function listModels() {
    ...
    if (result.error) throw new UpstreamError(errorMessage(result.error), result.response?.status ?? 502);
    const raw = result.data?.providers ?? [];
    const providers = Array.isArray(raw) ? raw : Object.entries(raw).map(([id, p]) => ({ ...(p as object), id }));

    const models: Array<{ id: string; owned_by: string; name: string }> = [];
    for (const provider of providers as Array<{ id: string; models?: Record<string, { name?: string }> }>) {
      for (const [modelId, model] of Object.entries(provider.models ?? {})) {
        models.push({ id: `${provider.id}/${modelId}`, owned_by: provider.id, name: model.name ?? modelId });
      }
    }
  ...
  app.get('/v1/models', async (_req, res) => {
    try {
      ...
      res.json({
        object: 'list',
        data: models.map((m) => ({ id: m.id, object: 'model', created: 0, owned_by: m.owned_by, name: m.name })),
      });
    } catch (err) {
      console.error(`[models] ${(err as Error).message}`);
      const status = err instanceof UpstreamError ? err.status : 502;
      res.status(status).json({ error: { message: errorMessage(err), type: 'upstream_error' } });
    }
  });
  ...
  app.post('/v1/chat/completions', async (req: Request, res: Response) => {
    ...
    if (parts.length === 0) {
      res.status(400).json({ error: { message: 'No user/assistant content to send', type: 'invalid_request_error' } });
      return;
    }
    ...
    try {
      ...
      if (body.stream) {
        await handleStream(res, { id, model: modelName, promptParams, sessionId });
      } else {
      ...
      const status =
        err instanceof UpstreamError ? err.status : message.includes('timed out') ? 504 : 500;
      if (!res.headersSent) {
        res.status(status).json({ error: { message, type: 'proxy_error' } });
      } else {
        writeSse(res, buildChunk({ id, model: modelName, delta: { content: `\n[proxy error] ${message}` }, finishReason: 'stop' }));
        res.write('data: [DONE]\n\n');
        res.end();
      }
    } finally {
  ...
  async function handleNonStream(
    ...
    const result = await withTimeout(client.session.prompt(ctx.promptParams), config.requestTimeoutMs, 'prompt');
    if (result.error) throw new UpstreamError(errorMessage(result.error), result.response?.status ?? 502);
    ...
    if (info?.error && !content && !reasoning) {
      res.status(502).json({ error: { message: errorMessage(info.error), type: info.error.name ?? 'upstream_error' } });
      return;
    }
  ...
  async function handleStream(
    res: Response,
    ctx: { id: string; model: string; sessionId: string; promptParams: PromptArgs },
  ) {
    res.setHeader('Content-Type', 'text/event-stream');
    res.setHeader('Cache-Control', 'no-cache');
    res.setHeader('Connection', 'keep-alive');
    res.flushHeaders?.();
    writeSse(res, buildChunk({ id: ctx.id, model: ctx.model, delta: { role: 'assistant' }, finishReason: null }));
      ...
      try {
        for await (const event of subscription.stream) {
          ...
          if (part.type === 'reasoning' && part.sessionID === ctx.sessionId) {
            sentReasoning += delta;
            writeSse(res, buildChunk({ id: ctx.id, model: ctx.model, delta: { reasoning_content: delta }, finishReason: null }));
          } else if (part.type === 'text' && part.sessionID === ctx.sessionId) {
            sentContent += delta;
            writeSse(res, buildChunk({ id: ctx.id, model: ctx.model, delta: { content: delta }, finishReason: null }));
          }
        }
      } catch {
    ...
    const remainderReasoning = reasoning.startsWith(sentReasoning) ? reasoning.slice(sentReasoning.length) : '';
    if (remainderReasoning) {
      writeSse(res, buildChunk({ id: ctx.id, model: ctx.model, delta: { reasoning_content: remainderReasoning }, finishReason: null }));
    }
    const remainderContent = content.startsWith(sentContent) ? content.slice(sentContent.length) : '';
    if (remainderContent) {
      writeSse(res, buildChunk({ id: ctx.id, model: ctx.model, delta: { content: remainderContent }, finishReason: null }));
    }

    writeSse(res, buildChunk({ id: ctx.id, model: ctx.model, delta: {}, finishReason: mapFinishReason(info?.finish) }));
    res.write('data: [DONE]\n\n');
    res.end();
  }

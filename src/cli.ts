...
function childEnv(opts: StartOptions): NodeJS.ProcessEnv {
  ...
  if (opts.opencodeUrl !== undefined)
    env.OPENCODE_SERVER_URL = opts.opencodeUrl;
  if (opts.opencodePort !== undefined)
    env.OPENCODE_PORT = String(opts.opencodePort);
  if (opts.requestTimeoutMs !== undefined)
    env.REQUEST_TIMEOUT_MS = String(opts.requestTimeoutMs);
  return env;
}
...
function resolveTarget(
  ...
  return { port: port ?? defaults.port, host: host ?? defaults.host };
}
...
async function cmdStart(opts: StartOptions): Promise<number> {
  ...
  if (existing && isAlive(existing.pid)) {
    console.error(
      `oc-openai is already running on port ${port} (pid ${existing.pid}).`,
    );
    return 1;
  }
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (await isHealthy(host, port)) {
      ...
      console.log(
        `oc-openai running on http://${dialHost(host)}:${port} (pid ${child.pid})`,
      );
  ...
  console.log(
    `Check status with 'oc-openai status' or inspect the log: ${log}`,
  );
  return 0;
}
...
async function cmdStop(port: number): Promise<number> {
  ...
  if (!isAlive(instance.pid)) {
    console.log(
      `oc-openai on port ${port} was not running; cleaning up state.`,
    );
    removeInstance(port);
    return 0;
  }
  ...
  try {
    ...
    console.error(
      `Failed to signal pid ${instance.pid}: ${(err as Error).message}`,
    );
    return 1;
  }
...
async function cmdStatus(port: number): Promise<number> {
  ...
  if (!isAlive(instance.pid)) {
    console.log(
      `oc-openai: stopped (stale state for pid ${instance.pid} on port ${port}).`,
    );
    return 1;
  }
...
async function main(): Promise<number> {
  ...
  const startOpts: StartOptions = {
    ...
    opencodePort:
      values['opencode-port'] !== undefined
        ? Number(values['opencode-port'])
        : undefined,
    requestTimeoutMs:
      values['request-timeout'] !== undefined
        ? Number(values['request-timeout'])
        : undefined,
  ...
  switch (command) {
    ...
    case 'restart': {
      ...
      await cmdStop(port);
      await sleep(500);
      return await cmdStart({ ...startOpts, foreground: false });
    }
    case 'logs': {
      const { port } = resolveTarget(portFlag, values.host);
